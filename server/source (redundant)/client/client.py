...
# FUNCTIONS
srv = pysftp.Connection(host="192.168.242.145", username="camera1", private_key="./rsakey.txt", cnopts=cnopts)
data = srv.listdir()
srv.close()
for i in data:
    print(i)
...
#send picture to server using pysftp
'''
def connect_server_send( file_name: str , file_data: bytes ) -> bool:
    with pysftp.Connection(host="192.168.242.145", username="camera1", private_key="./rsakey.txt") as sftp:
        sftp.cwd('/root/public')  # The full path
        sftp.put(file_name, file_data)  # Upload the file
'''
...
while True: # Main function
    try:  
        my_image = get_picture()  # get picture to use pickle to convert to byte array
